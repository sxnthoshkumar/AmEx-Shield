...
import { Shield, Lock, AlertTriangle, CheckCircle2, Zap, Activity, Award } from 'lucide-react';
...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="relative overflow-hidden rounded-2xl glass-blue-card p-8 lg:p-10 shadow-2xl">
        <div className="absolute top-0 right-0 w-96 h-96 bg-sky-500/10 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-3 max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-300 text-xs font-mono tracking-wider">
          ...
          <div className="shrink-0">
            <button
              ...
              className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-600 hover:from-sky-300 hover:to-cyan-300 text-black font-extrabold text-sm tracking-wide shadow-glow-cyan transition flex items-center gap-2.5"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-card rounded-2xl p-6 space-y-3">
          ...
          <div className="text-3xl font-extrabold text-cyan-300 font-mono">{pendingHitlCount}</div>
          <div className="text-xs text-slate-400">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {passports.map((passport) => {
              ...
              const progressBg = passport.trustScore >= 90 ? 'bg-emerald-500' : passport.trustScore >= 75 ? 'bg-cyan-500' : 'bg-red-500';
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-slate-900 text-cyan-300 border border-cyan-500/20">
                        {passport.category}
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs pt-1 border-t border-white/5">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-cyan-300">${passport.dailyRiskBudgetRemaining.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="glass-card rounded-2xl p-5 space-y-4 max-h-[540px] overflow-y-auto">
            {evaluations.length === 0 ? (
              <div className="text-center py-16 space-y-3">
                ...
                <button
                  onClick={onLaunchSimulator}
                  className="px-4 py-2 rounded-xl bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 text-xs font-mono font-bold"
                >
                  Run Agent Sandbox
                </button>
              </div>
